...
function selectProvider(config: ModelConfig | null): string | null {
  if (!config) {
    return null;
  }

  const current = normalizeProvider(config.current_provider);
  if (current) {
    return current;
  }

  const firstModel = Array.isArray(config.models) ? config.models[0] : null;
  const fromModel = normalizeProvider(firstModel?.provider);
  if (fromModel) {
    return fromModel;
  }
...
export async function clearModelOverride(): Promise<ModelConfigWriteResult> {
  ...
  delete merged.current_provider;
...
export async function getModelStatus(): Promise<ModelStatus> {
  ...
  const resolvedProvider = selectProvider(data);
  const models = Array.isArray(data?.models)
    ? data?.models
        .map((item) => normalizeModel(item?.name))
        .filter((name): name is string => Boolean(name))
    : undefined;
  const providers = data?.providers ? Object.keys(data.providers) : undefined;

  return {
    path,
    currentModel: normalizeModel(data?.current_model) ?? undefined,
    currentProvider: normalizeProvider(data?.current_provider) ?? undefined,
...
export async function resolveProviderForRun(_platformKey: string): Promise<string | undefined> {
  const envPath = process.env.PULSE_CODER_MODEL_CONFIG?.trim();
  const configPath = await findConfigPath();
  const path = envPath || configPath || null;
  if (!path) {
  ...
  try {
    const stat = await fs.stat(path);
    if (cachedConfig && cachedConfig.path === path && cachedConfig.mtimeMs === stat.mtimeMs) {
      return selectProvider(cachedConfig.data) ?? undefined;
    }
    ...
    cachedConfig = { path, mtimeMs: stat.mtimeMs, data };
    return selectProvider(data) ?? undefined;
  } catch {
    return undefined;
  }
}

export async function getProviderConfig(providerKey: string): Promise<ProviderConfig | undefined> {
  const envPath = process.env.PULSE_CODER_MODEL_CONFIG?.trim();
  const configPath = await findConfigPath();
  const path = envPath || configPath || null;
  if (!path) {
  ...
  try {
    ...
    return data.providers[providerKey];
  } catch {
    return undefined;
  }
}
